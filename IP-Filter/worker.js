...
const REGION_MAP = {
    // 亚太地区 (Asia Pacific)
    'CN': '中国大陆', 'JP': '日本', 'KR': '韩国', 'SG': '新加坡', 'HK': '香港', 'TW': '台湾',
...
export default {
    async fetch(request, env) {
        ...
        if (url.pathname === '/admin' || url.pathname === '/admin/') {
            // 获取密码，默认为 666666
            const expectedPassword = env.PASSWORD || '666666';
            const authHeader = request.headers.get('Authorization');

            if (!authHeader) {
                return new Response('Unauthorized: 需要输入密码', {
                    status: 401,
            ...
            try {
                ...
                if (password !== expectedPassword) {
                    return new Response('Unauthorized: 密码错误', {
                        status: 401,
                        headers: { 'WWW-Authenticate': 'Basic realm="Admin Login"' }
                    });
                }
...
async function handleGetRegions() {
    try {
        ...
        const counts = {};
        matches.forEach(tag => {
            const region = tag.replace('#', '').trim().toUpperCase();
            // 增加严谨过滤：标准国家码必须恰好是2位纯字母，拒绝 "U" 或 "UPDATED" 这种脏数据
            if (region.length === 2) {
